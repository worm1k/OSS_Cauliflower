...
public class RegistrationController extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(userRoleId<=0){
            request.getSession().setAttribute(CauliflowerInfo.ERROR_ATTRIBUTE, "Incorrect user role");
        ...
        if(email==null || email.length()<6){
            request.getSession().setAttribute(CauliflowerInfo.ERROR_ATTRIBUTE, "Incorrect e-mail");
            response.sendRedirect(pathFrom);
        }
        password = (String) request.getParameter("password");
        if(password==null || password.length()<6){
            request.getSession().setAttribute(CauliflowerInfo.ERROR_ATTRIBUTE,"Incorrect password");
            response.sendRedirect(pathFrom);
        }
        firstName = (String) request.getParameter("name");
        if(firstName==null || firstName.length()<2){
            request.getSession().setAttribute(CauliflowerInfo.ERROR_ATTRIBUTE,"Incorrect name");
            response.sendRedirect(pathFrom);
        }
        lastName = (String) request.getParameter("surname");
        if(lastName==null || lastName.length()<2){
            request.getSession().setAttribute(CauliflowerInfo.ERROR_ATTRIBUTE,"Incorrect surname");
            response.sendRedirect(pathFrom);
        }
        phone = (String) request.getParameter("phone");
        if(phone==null || phone.length()<10){
            request.getSession().setAttribute(CauliflowerInfo.ERROR_ATTRIBUTE,"Incorrect phone");
            response.sendRedirect(pathFrom);
        }
        ...
        if(DAO.INSTANCE.checkForEmailUniq(email)){
            if(DAO.INSTANCE.checkForPhoneUniq(phone)) {
                ...
                request.getSession().setAttribute(CauliflowerInfo.ERROR_ATTRIBUTE,"User with this phone already exist");
                response.sendRedirect(pathFrom);
            }
        }else{
            request.getSession().setAttribute(CauliflowerInfo.ERROR_ATTRIBUTE,"User with this e-mail already exist");
            response.sendRedirect(pathFrom);
        }
    }
